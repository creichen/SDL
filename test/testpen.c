...
DrawScreen(SDL_Renderer *renderer)
{
    ...
    if (last_was_eraser) {
        ...
        SDL_RenderDrawLineF(renderer,
                            last_x - 10 - distance, last_y,
                            last_x - distance, last_y);
        SDL_RenderDrawLineF(renderer,
                            last_x + 10 + distance, last_y,
                            last_x + distance, last_y);
        SDL_RenderDrawLineF(renderer,
                            last_x, last_y - 10 - distance,
                            last_x, last_y - distance);
        SDL_RenderDrawLineF(renderer,
                            last_x, last_y + 10 + distance,
                            last_x, last_y + distance);
    }
    ...
    if (fabs(last_xtilt) < 0.1f && fabs(last_ytilt) < 0.1f) {
        ...
        for (rot = 0; rot < 4; ++rot) {
            ...
            float px = last_x  + vecx * pressure;
            float py = last_y  + vecy * pressure;
